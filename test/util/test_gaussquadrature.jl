@testsnippet GaussQuadratureSetup begin
    using TransportMaps
    using Test

end

@testitem "Gauss Quadrature" setup = [GaussQuadratureSetup] begin
    @testset "Basic Integration Tests" begin
        ...
        @test result ≈ 2.0 atol = 1.0e-10  # ∫₀² x dx = [x²/2]₀² = 2

        result = gaussquadrature(f_linear, 10, -1.0, 1.0)
        @test result ≈ 0.0 atol = 1.0e-10  # ∫₋₁¹ x dx = 0 (odd function)

        # Test integration of quadratic function
        f_quad(x) = x^2
        result = gaussquadrature(f_quad, 5, 0.0, 1.0)
        @test result ≈ 1 / 3 atol = 1.0e-10  # ∫₀¹ x² dx = [x³/3]₀¹ = 1/3

        result = gaussquadrature(f_quad, 10, -2.0, 2.0)
        @test result ≈ 16 / 3 atol = 1.0e-10  # ∫₋₂² x² dx = [x³/3]₋₂² = 8/3 - (-8/3) = 16/3
    end
    ...
    @testset "Polynomial Integration" begin
        ...
        @test result ≈ expected atol = 1.0e-12
    ...
    @testset "Edge Cases" begin
        ...
        @test result_forward ≈ -result_backward atol = 1.0e-15
