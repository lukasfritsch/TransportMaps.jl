...
@testset "Gauss Quadrature" begin
    ...
    @testset "Convergence" begin
        # Test that increasing the number of points improves accuracy
        f(x) = exp(x) * sin(x)
        true_value = gaussquadrature(f, 50, 0.0, 1.0)  # High-precision reference
        
        error_5 = abs(gaussquadrature(f, 5, 0.0, 1.0) - true_value)
        error_10 = abs(gaussquadrature(f, 10, 0.0, 1.0) - true_value)
        error_20 = abs(gaussquadrature(f, 20, 0.0, 1.0) - true_value)
        
        @test error_10 < error_5
        @test error_20 < error_10
    end
end